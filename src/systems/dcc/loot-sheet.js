...
import QuantityDialog from "../../dialogs/quantity-dialog.js";
import config from "../../config.js";
import CurrencyHelper from "./currency-helper.js";
//import Item5e from "../../systems/dnd5e/module/item/entity.js";

export default class LootSheet extends DCCActorSheet {
    get template() {


        if (!game.user.isGM && this.actor.limited) {
            return `systems/dcc/templates/actor-sheet-npc.html`;
        }
        return "modules/fvtt-loot-sheet-npc-dcc/template/npc-sheet.html";
    }
    ...
    async getData() {
        ...
        let lootsheettype = await this.actor.getFlag("fvtt-loot-sheet-npc-dcc", "lootsheettype");
        if (!lootsheettype) await this.actor.setFlag("fvtt-loot-sheet-npc-dcc", "lootsheettype", "Loot");
        lootsheettype = await this.actor.getFlag("fvtt-loot-sheet-npc-dcc", "lootsheettype");


        let priceModifier = 1.0;
        if (lootsheettype === "Merchant") {
            priceModifier = await this.actor.getFlag("fvtt-loot-sheet-npc-dcc", "priceModifier");
            if (!priceModifier) await this.actor.setFlag("fvtt-loot-sheet-npc-dcc", "priceModifier", 1.0);
            priceModifier = await this.actor.getFlag("fvtt-loot-sheet-npc-dcc", "priceModifier");
        }

        let totalWeight = 0;
        this.actor.data.items.forEach((item)=>
            totalWeight += Math.round((item.data.quantity * item.data.weight * 100) / 100));

        let totalPrice = 0;
        this.actor.data.items.forEach((item)=>
            totalPrice += Math.round(item.data.quantity * CurrencyHelper.convertCurrencyObjectToCopper(item.data.value) * priceModifier))
        ...
        console.log('Loot Items | Sheet Data')
    ...
    async _merchantSettingChange(event, html) {
        ...
        const moduleNamespace = "fvtt-loot-sheet-npc-dcc";
    ...
    async _merchantInventoryUpdate(event, html) {
        ...
        const reducedVerbosity = game.settings.get("fvtt-loot-sheet-npc-dcc", "reduceUpdateVerbosity");
    ...
    async _changeSheetType(event, html) {
        ...
        await currentActor.setFlag("fvtt-loot-sheet-npc-dcc", "lootsheettype", selectedItem);
    ...
    _lootCoins(event) {
        event.preventDefault();
        if (!game.settings.get("fvtt-loot-sheet-npc-dcc", "lootCurrency")) {
            return;
        }
    ...
    async _priceModifier(event) {
        ...
        let priceModifier = await this.actor.getFlag("fvtt-loot-sheet-npc-dcc", "priceModifier");
        ...
        renderTemplate("modules/fvtt-loot-sheet-npc-dcc/template/price-modifier-dialog.html", {priceModifier: priceModifier}).then(html => {
            new Dialog({
                ...
                buttons: {
                    one: {
                        ...
                        label: "Update",
                        callback: () => {
                            const value = document.getElementById("price-modifier-percent").value / 100
                            console.log(value)
                            this.actor.setFlag("fvtt-loot-sheet-npc-dcc", "priceModifier", value)
                        }
                    },
                    two: {
    ...
    _hackydistributeCoins(containerActor) {
        //This is identical as the distributeCoins function defined in the init hook which for some reason can't be called from the above _distributeCoins method of the fvtt-loot-sheet-npc-dcc class. I couldn't be bothered to figure out why a socket can't be called as the GM... so this is a hack but it works.
    ...
    _prepareItems(actorData) {
        ...
        let items = actorData.actor.items;
    ...
    _prepareGMSettings(actorData) {
        ...
        for (let player of players)
        {
            ...
            if (actor) {
                ...
                player.lootPermission = getLootPermissionForPlayer(actorData, player);
